...
describe('cryptonote_utils tests', function () {
  it('create_address aka address_and_keys_from_seed', async function () {
    ...
    const decoded = monero_utils.address_and_keys_from_seed('9c973aa296b79bbf452781dd3d32ad7f', nettype)
    assert.strictEqual(
  ...
  it('decode mainnet primary address', async function () {
    const monero_utils = await require('../monero_utils/MyMoneroCoreBridge')({})
    const decoded = monero_utils.decode_address(
    ...
    const expected = {
      ...
      spend: 'd8f1e81ecbe25ce8b596d426fb02fe7b1d4bb8d14c06b3d3e371a60eeea99534',
      view: '576f0e61e250d941746ed147f602b5eb1ea250ca385b028a935e166e18f74bd7'
    }
  ...
  it('generate key derivation', async function () {
    const monero_utils = await require('../monero_utils/MyMoneroCoreBridge')({})
    const derivation = monero_utils.generate_key_derivation(
      public_key,
      private_key
    )
  ...
  it('derive public key', async function () {
    ...
    const output_key = monero_utils.derive_public_key(
      derivation,
      1,
      public_key
    )
  ...
  it('derive subaddress public key', async function () {
    ...
    const subaddress_public_key = monero_utils.derive_subaddress_public_key(
      public_key,
      derivation,
      1
    )
  ...
  it('decodeRct', async function () {
    ...
    const ret = monero_utils.decodeRct(rv, sk, i)
  ...
  it('estimate_fee', async function () {
    const monero_utils = await require('../monero_utils/MyMoneroCoreBridge')({})
    const fee = monero_utils.estimate_fee({
  ...
  it('estimate_tx_weight', async function () {
    const monero_utils = await require('../monero_utils/MyMoneroCoreBridge')({})
    const weight = monero_utils.estimate_tx_weight({
      use_rct: true,
      n_inputs: 2,
      mixin: 10,
      n_outputs: 2,
